...
async function main() {
  ...
  // Proton SMTP config - you need to enable smtp submission token in your proton account
  const transporter = nodemailer.createTransport({
    host: 'smtp.protonmail.ch',
    port: 587,
    secure: false, // STARTTLS
    auth: {
      user: 'your protonmail smtp submission address',
      pass: 'your smtp generated token', // not your password!!
    },
  });

  const mailOptions = {
    from: '"Lumo-Api" <your protonmail smtp submission address>',
    to,
    subject,
    html: `<pre style="font-family: monospace;">${body}</pre>`,
    text: body.replace(/<\/?[^>]+(>|$)/g, ''), // plain text for fallback
  };
