...
class LumoApp:
    ...
    def build_widgets(self) -> None:
        ...
        self.toggle_websearch_btn.pack(side=tk.TOP, padx=5, pady=5)
        self.enable_ghost_btn = ttk.Button(
            control_frame, text="Enable Ghost Mode 
", command=self.enable_ghost_mode
        )
        self.enable_ghost_btn.pack(side=tk.TOP, padx=5, pady=5)
        self.disable_ghost_btn = ttk.Button(
            control_frame, text="Disable Ghost Mode 
", command=self.disable_ghost_mode
        )
        self.disable_ghost_btn.pack(side=tk.TOP, padx=5, pady=5)
        self.new_chat_btn = ttk.Button(
            control_frame, text="Start New Chat 
", command=self.start_new_chat
        )
        self.new_chat_btn.pack(side=tk.TOP, padx=5, pady=5)
        self.upload_file_btn = ttk.Button(
            control_frame, text="Upload File", command=self.upload_file
        )
        self.upload_file_btn.pack(side=tk.TOP, padx=5, pady=5)
        self.remove_single_file_btn = ttk.Button(
            control_frame, text="Remove Single File", command=self.remove_single_file
        )
        self.remove_single_file_btn.pack(side=tk.TOP, padx=5, pady=5)
        self.remove_all_files_btn = ttk.Button(
            control_frame, text="Remove All Files", command=self.remove_all_files
        )
        self.remove_all_files_btn.pack(side=tk.TOP, padx=5, pady=5)
        # chat logs
        chat_logging_frame = ttk.Frame(sidebar_frame)
        chat_logging_frame.pack(side=tk.TOP, padx=5, pady=5)
        ...
        self.toggle_chat_logging_btn = ttk.Button(
            sidebar_frame, text="Enable Chat Logging", command=self.toggle_chat_logging
        )
        self.toggle_chat_logging_btn.pack(side=tk.TOP, padx=5, pady=5)
